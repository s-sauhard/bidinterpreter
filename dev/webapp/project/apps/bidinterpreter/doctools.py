...
class DocTools:
    ...
    def get_entity_matches(self, fulltext, wordregistry):
        ...
        for key, pattern in self.extract_regex.items():
            ...
            for match in re.finditer(pattern['segment'], fulltext, re.IGNORECASE):
                    ...
                    if type(wordregistry) == list:
                        wordregistry = dict(words = wordregistry)

                    for index, word in enumerate(wordregistry['words']):
                        success = 0
                        ...
                        if match.start() >= word['startindex'] and match.start() <= word['endindex']:
                            ...
                            print(type(word), word.keys())
                            if type(word['x0']) == list:
                                x0      = float(word['x0'][0])
                                x1      = float(word['x1'][0])
                                # y0      = float(word.get('y0', [0])[0])
                                # y1      = float(word.get('y1', [0])[0])
                                top     = float(word['top'][0])
                                bottom  = float(word['bottom'][0])
                            elif type(word['x0']) != float:
                                x0      = float(word['x0'])
                                x1      = float(word['x1'])
                                y0      = float(word.get('y0', 0))
                                y1      = float(word.get('y1', 9))
                                top     = float(word['top'])
    ...
    def get_entity_coordinates(self, entity_name = False, entity_value = False, from_index = False, segment_length = 10, word_coords = False):
        ...
        for index, word_coord in enumerate(word_coords['words'][from_index:from_index+segment_length]):
            ...
            if match:
                print("Word coords------", type(word_coords))
                found_match = dict(
                    index  = index,
                    entity_name = entity_name,
                    entity_value = entity_value,
                    x0     = word_coords['words'][from_index + index]['x0'],
                    x1     = word_coords['words'][from_index + index]['x1'],
                    y0     = word_coords['words'][from_index + index]['bottom'],
                    y1     = word_coords['words'][from_index + index]['top']
                )

                if single_match:
                    found_match['x1'] = word_coords['words'][from_index + index]['x1']
                else:
                    found_match['x1'] = word_coords['words'][from_index + index + entity_length - 1]['x1']
    ...
    def image_to_highlighted(self, highlights, pdfpath, mirrorimagepath):
        ...
        with pdfplumber.open(pdfpath) as pdf:
            ...
            for page in pdf.pages:
                ...
                if page.width > 0:
                    scale = float(base.size[0]/page.width) # I'm using this to translate pdf plumber coordinates to image coordinates
                else: 
                    scale = float(1)  #defaults to 1 in case there is no page width
